...
export function LanguagePicker({ surface = 'light', compact = false }: Props) {
                ...
                <li key={l} role="option" aria-selected={active}>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3.5 py-2.5 text-[13px] font-medium transition-colors ${
                      onDark ? 'hover:bg-white/[0.08]' : 'hover:bg-ink/[0.04]'
                    } ${active ? (onDark ? 'bg-white/[0.08]' : 'bg-ink/[0.04]') : ''}`}
                  >
                    <span className="block h-3.5 w-5 overflow-hidden rounded-[2px] shrink-0 ring-1 ring-black/10">
                      {FLAGS[l]}
                    </span>
                    <span className="flex-1 text-left">{t.nav.languages[l]}</span>
                    {active && (
                      <Check size={14} strokeWidth={2.5} className={onDark ? 'text-white/85' : 'text-ink'} />
                    )}
                  </button>
                </li>
              )
  ...
  return (
    <svg viewBox="0 0 60 30" className="block h-full w-full" preserveAspectRatio="none">
      ...
      <g clipPath="url(#lp-gb-clip)">
        ...
        <path d="M0,0 L60,30 M60,0 L0,30" clipPath="url(#lp-gb-clip2)" stroke="#C8102E" strokeWidth="4" />
